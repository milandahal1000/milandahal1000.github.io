...
function buildSidebar() {
  ...
  $("#file-tree").innerHTML = FILES.map(
    (f) =>
      `<li class="file-item ${f.name === currentFile ? "active" : ""}" data-file="${f.name}">
       <span class="dot ${f.icon}"></span>${f.name}</li>`,
  ).join("");

  $("#outline").innerHTML = FILES.map(
    (f) =>
      `<li class="file-item ${f.name === currentFile ? "active" : ""}" data-file="${f.name}">
       <span class="dot ${f.icon}"></span>${esc(f.title)}</li>`,
  ).join("");

  $("#timeline").innerHTML = (D.commits || [])
    .map(
      (c) =>
        `<li title="${esc(c.msg)}">✦ ${esc(c.msg)}<small>${esc(c.time)}</small></li>`,
    )
    .join("");

  $("#drawer-list").innerHTML = FILES.map(
    (f) =>
      `<li class="file-item ${f.name === currentFile ? "active" : ""}" data-file="${f.name}">
       <span class="dot ${f.icon}"></span>${f.name} <small>— ${esc(f.title)}</small></li>`,
  ).join("");
...
function openFile(name, smooth = true) {
  ...
  currentFile = name;
  $("#viewport").scrollTo({
    top: Math.max(0, panelTop(name) - 6),
...
function renderHome() {
  ...
  const stats = (P.stats || [])
    .map(
      (s) =>
        `<div class="stat"><b>${s.value}</b><span>${esc(s.label)}</span></div>`,
    )
    .join("");
  sec.innerHTML = `
    <div class="hero">
...
function startTypewriter() {
  ...
  (function tick() {
    ...
    if (!deleting) {
      ...
      if (c === 0) {
        deleting = false;
        r = (r + 1) % (P.roles || ["Developer"]).length;
      }
    }
...
function animateBars() {
  requestAnimationFrame(() => {
    $$("#panel-skills .sbar-fill").forEach((b) => {
...
function openProject(title) {
  ...
  const links = [
    p.links.github
      ? `<a class="btn-ghost" href="${esc(p.links.github)}" target="_blank" rel="noopener">🐙 View source</a>`
      : "",
    p.links.live
      ? `<a class="btn-ghost" href="${esc(p.links.live)}" target="_blank" rel="noopener">🔗 Live demo</a>`
      : "",
...
function buildSearchIndex() {
  const add = (file, text) => {
    if (text) SEARCH_INDEX.push({ file, text: text.toLowerCase(), raw: text });
  };
  add(
    "index.html",
...
function renderSearch(q) {
  ...
  const results = ql
    ? SEARCH_INDEX.filter((e) => e.text.includes(ql))
        .map((e) => ({ ...e, idx: e.text.indexOf(ql) }))
        .sort((x, y) => x.idx - y.idx)
        .slice(0, 15)
    : [];
  $("#search-count").textContent = ql
    ? `${results.length} result${results.length === 1 ? "" : "s"} (of ${SEARCH_INDEX.length} entries)`
  ...
  list.innerHTML = results.length
    ? results
      ...
      <span class="sf-go">→</span></li>`;
        ...
        .join("")
    : ql
      ? '<li class="search-empty">No matches found</li>'
      : '<li class="search-empty">Try "react", "backend", "university"…</li>';
}
...
function buildCommands() {
  ...
  cmds.push(
    { label: "Toggle Terminal", hint: "Ctrl+`", run: () => toggleTerminal() },
    {
      ...
      hint: "Ctrl+B",
      run: () => document.body.classList.toggle("mini"),
    },
    {
      label:
    ...
    {
      label: "Hire Milan Dahal",
      hint: "!",
      run: () => openFile("contact.html"),
    },
    {
...
function renderPalette(list) {
  ...
  el.innerHTML =
    palFiltered
      ...
      .join("") || '<li class="search-empty">No matching commands</li>';
}
/* ─── Terminal ─── */
...
function runCommand(raw) {
  ...
  switch (cmd.split(" ")[0]) {
    ...
    case "whoami":
      tprint(
        `${p.name} — ${D.about.role} from ${p.location} ${D.meta.flag}`,
        "ok",
      );
      break;
    case "about":
      (D.about.paragraphs || []).forEach((x) => tprint(x, "info"));
      break;
    case "experience":
      (D.experience || []).forEach((e) =>
        tprint(`${e.period} — ${e.role} @ ${e.company}`, "ok"),
      );
      break;
    case "skills":
    ...
    case "projects":
      (D.projects || []).forEach((x) =>
        tprint(`${x.emoji} ${x.title} [${x.category}]`, "ok"),
      );
      break;
    case "education":
      tprint(
    ...
    case "email":
      ...
      break;
    case "socials":
      tprint("GitHub: " + GITHUB, "ok");
      tprint("LinkedIn: " + LINKEDIN, "ok");
      break;
    case "resume":
      tprint("Generating resume... 📄", "info");
      downloadResume();
      break;
    case "theme": {
    ...
    case "matrix":
      ...
      break;
    case "hire":
      tprint("Excellent choice! Opening contact form... 🚀", "ok");
    ...
    case "clear":
      ...
      break;
    case "date":
      tprint(kathmanduTime(), "ok");
      break;
    default:
      tprint(`command not found: ${cmd} — type 'help'`, "err");
  }
}
function matrixRain() {
  const body = $("#terminal-body");
  ...
  const iv = setInterval(() => {
    ...
    const line = document.createElement("div");
    line.className = "t-line matrix";
    line.textContent = row;
    body.appendChild(line);
    body.scrollTop = body.scrollHeight;
    if (++runs > 30) {
      clearInterval(iv);
      tprint("Wake up, Neo... ☕", "ok");
    }
  }, 70);
}
const kathmanduTime = () =>
...
function toggleDark() {
  ...
  toast(
    "success",
    settings.darkMode ? "Dark mode enabled 🌙" : "Light mode enabled ☀️",
  );
}
function setAccent(id) {
  const t = THEMES.find((x) => x.id === id);
  if (!t) {
    tprint(`Unknown theme: ${id}`, "err");
    return;
  }
  settings.accent = id;
  applySettings();
  ...
  const swatches = THEMES.map(
    (t) =>
      `<button class="theme-swatch ${t.id === settings.accent ? "active" : ""}" data-theme="${t.id}" title="${t.name}"
       style="--sw: ${t.accent}"><i></i><span>${t.name}</span></button>`,
  ).join("");
...
document.addEventListener("click", (e) => {
  ...
  if (ac) {
    const f = runAction(ac.dataset.action);
    if (f) f();
    return;
  }
  // Project card
  ...
  if (pc) {
    openProject(pc.dataset.title);
    return;
  }
  // Project filter
  const pf = e.target.closest(".proj-filter[data-cat]");
  if (pf) {
    projFilter = pf.dataset.cat;
    renderProjects();
    return;
  }
  // Palette item
  ...
  if (pi) {
    const c = palFiltered[+pi.dataset.i];
    if (c) {
      closePalette();
      c.run();
    }
...
$("#palette-input").addEventListener("keydown", (e) => {
  ...
  if (e.key === "Enter" && palFiltered[palIndex]) {
    const c = palFiltered[palIndex];
    closePalette();
    c.run();
    e.preventDefault();
  }
...
$("#settings-body").addEventListener("click", (e) => {
  ...
  if (e.target.id === "set-terminal") {
    settings.terminal = e.target.checked;
    saveSettings();
    applySettings();
  }
  if (e.target.id === "set-anim") {
...
$("#viewport").addEventListener("scroll", () => {
  updateScrollTop();
  if (!sbHovered) syncSidebarFromView();
  updateActiveSection();
});
/* Scrolling the sidebar → editor follows */
$("#sidebar").addEventListener("scroll", () => {
  if (sbHovered && !vpHovered) syncViewFromSidebar();
});
$("#viewport").addEventListener("mousemove", (e) => {
  const rect = $("#viewport").getBoundingClientRect();
  const ln = Math.max(
    1,
    Math.round((e.clientY - rect.top + $("#viewport").scrollTop) / 20),
  );
...
async function fetchGitHub() {
  try {
    ...
    const av = $(".pc-avatar");
    if (av && u.avatar_url) {
      liveAvatar = `<img src="${u.avatar_url}" alt="GitHub avatar" onerror="this.style.display='none'"/>`;
      av.innerHTML = liveAvatar;
    }
    if (u.public_repos) {
...
async function fetchGitHubProjects() {
  try {
    ...
    if (
      githubProjectsCache &&
      now - githubProjectsCacheTimestamp < GITHUB_CACHE_DURATION
    ) {
      // Use cached data
      ...
      notify(
        ...
        `Showing ${githubProjectsCache.length} cached projects from GitHub`,
        "projects.json",
      );
      return;
    }
    ...
    const response = await fetch(
      "https://api.github.com/users/milandahal1000/repos?sort=updated&direction=desc&per_page=100",
      {
        headers: {
          "User-Agent": "Milan-Portfolio/1.0",
        },
    ...
    const projects = filteredRepos.map((repo) => {
      ...
      if (
        ...
        if (
          ["javascript", "typescript", "html", "css", "vue", "svelte"].includes(
            lang,
          )
        ) {
          category = "Frontend";
    ...
    notify(
      "success",
      "Projects updated",
      `Loaded ${projects.length} projects from GitHub`,
      "projects.json",
    );
  } catch (error) {
